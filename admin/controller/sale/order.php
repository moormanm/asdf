...
class ControllerSaleOrder extends Controller {
  	...
  	public function index() {
        ...
        public function changeStatus() {
           ...
           if ($this->request->server['REQUEST_METHOD'] == 'POST')  {
                ...
                if($this->isValidOrderStatusID($_POST['newStatus']))  {
                   $order_info = $this->model_sale_order->updateOrderStatus(intval($_POST['order_id']), intval($_POST['newStatus']) );
                }
                $this->request->get['order_id'] = $_POST['order_id'];
                $this->info();
        }
	...
	public function info() {
		...
		if ($order_info) {
			...
			$this->data['text_datePurchased'] = $this->language->get('text_datePurchased');
			...
			if (isset($this->request->get['filter_datePurchased'])) {
				$url .= '&filter_datePurchased=' . $this->request->get['filter_datePurchased'];
			}
			
			if (isset($this->request->get['filter_dateModified'])) {
				$url .= '&filter_dateModified=' . $this->request->get['filter_dateModified'];
			}
			...
			$this->data['delete'] = $this->url->link('sale/order/delete', 'token=' . $this->session->data['token'] .  '&order_id=' . (int)$this->request->get['order_id'] , 'SSL');
			...
			$this->data['contactNumber'] = $order_info['contactNumber'];
						
			$this->data['email'] = $order_info['email'];
			$this->data['total'] = $this->currency->format($order_info['total']);
			
			$this->load->model('localisation/order_status');
			$order_status_info = $this->model_localisation_order_status->getOrderStatus($order_info['order_status']);
			...
			$this->data['order_statuses'] = $this->model_localisation_order_status->getOrderStatuses();
                         ...
                         $this->data['changeStatusAction'] = $this->url->link('sale/order/changeStatus', 'token=' . $this->session->data['token'] . $url, 'SSL');



			$this->data['order_status_id'] = $order_info['order_status'];
  	...
  	protected function getList() {
		...
		if (isset($this->request->get['filter_datePurchased'])) {
			$filter_datePurchased = $this->request->get['filter_datePurchased'];
		} else {
			$filter_datePurchased = null;
		}
		...
		$data = array(
			...
			'filter_datePurchased'      => $filter_datePurchased,
    	...
    	foreach ($results as $result) {
			...
			if (strtotime($result['datePurchased']) > strtotime('-' . (int)$this->config->get('config_order_edit') . ' day')) {
				$action[] = array(
			...
			$this->data['orders'][] = array(
				...
				'datePurchased'    => date($this->language->get('date_format_short'), strtotime($result['datePurchased'])),
		...
		$this->data['sort_datePurchased'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=o.datePurchased' . $url, 'SSL');
		...
		$this->data['filter_datePurchased'] = $filter_datePurchased;
