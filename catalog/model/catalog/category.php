...
class ModelCatalogCategory extends Model {
	public function getCategory($category_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "category c " .
                "LEFT JOIN " . DB_PREFIX . "category_description cd ON " .
                "(c.category_id = cd.category_id) WHERE " . 
                "c.category_id = '" . (int)$category_id . "' AND " .
                "c.status = '1'");
	...
	public function getCategories($parent_id = 0) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "category c " . 
                ...
                "WHERE status = '1' ORDER BY c.sort_order, LCASE(cd.name)");
